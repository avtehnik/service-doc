 ...
 * ~~~
 */
class YiiServiceDocAction extends Action
{
    /**
    ...
    public function run()
    {
        ...
        if ($this->cache !== null) {
            $cache = $this->getCache();
            if (($serviceDoc = $cache->get($this->cacheKey)) === false) {
                $serviceDoc = $this->getServiceDoc();
                $cache->set($this->cacheKey, $serviceDoc);
            }
        } else {
            $serviceDoc = $this->getServiceDoc();
        }
     ...
     */
    protected function getServiceDoc()
    {
        return \ServiceDoc\scan($this->scanDir, $this->scanOptions);
    }
}
